...
A demo **tool-mode trik** that computes and compares cryptographic hashes. No LLM, no conversation, no handoff — just pure tools that appear as native capabilities on the consuming agent.
...
```bash
trik install @molefas/trik-hash
```
...
Once installed, the tools appear directly on the consuming agent:

```
User: What's the SHA-256 hash of "hello world"?
Agent: sha256: b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9

User: Does "hello world" match hash abc123 with md5?
Agent: Hash md5 match: false
```
